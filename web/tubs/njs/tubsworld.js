...
function loadCircuit(xmlString) {
...
function writedata() {
    ...
    for (gateIndex = 0; gateIndex < circuitXML.gates.length; gateIndex++) {
        ...
        db += circuitXML.gates[gateIndex].toString() + "\n";

        for (gp = 0; gp < circuitXML.gates[gateIndex].paths.length; gp++) {

            db += circuitXML.gates[gateIndex].paths[gp].toString() + "\n";
            for (cp = 0; cp < circuitXML.gates[gateIndex].paths[gp].cells.length; cp++) {
                db += circuitXML.gates[gateIndex].paths[gp].cells[cp].toString() + "\n";
            }
...
function generateView() {
    ...
    for (p = 0; p < circuitXML.paths.length; p++) {
        ...
        //drawPolyBezier();
        //gr.drawText(circuitXML.paths[pathIndex].pathName, new jsPoint(absx, absy), "", new jsColor("red"), "0.2", "center");
