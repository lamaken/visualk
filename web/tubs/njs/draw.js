...
function drawIcons() {
    ...
    for (var p = 0; p < circuitXML.paths.length; p++) {
        for (var c = 0; c < circuitXML.paths[p].cells.length; c++) {
            ...
            var cx = parseInt((((px + 1.0) / 2.0) * (canvas.width - 40)));
            var cy = parseInt((((1.0 - py) / 2.0) * (canvas.height - 40)));
            if (circuitXML.paths[p].cells[c].type === 'end') {
                drawHome(cx+20  - (72 / 2 / 2), cy+20  - (72 / 2 / 2));
            }
            if (circuitXML.paths[p].cells[c].type === 'kill') {
                drawDead(cx+20  - (72 / 2 / 2), cy+20  - (72 / 2 / 2));
            }
...
function drawDead(x, y) {
    context.drawImage(dead_img, x, y, 72 / 2, 72 / 2);
}
function drawHome(x, y) {
    context.drawImage(home_img, x, y, 72 / 2, 72 / 2);
}
function drawSwap() {
    drawPaths();
    drawGatesSwap();
